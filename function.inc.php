...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($err)) {
    ...
    if (empty($url) || !preg_match('/^https?:\/\/(www\.)?(youtube\.com|youtu\.be)\//', $url)) {
        ...
        // Example: yt_6725a13b.mp4 or yt_6725a13b.mp3
        $safeFile = uniqid('yt_', false) . '.' . $format;
        $opTemp = "$dlDir/{$safeFile}";
        ...
        if ($returnCode === 0) {
            if (file_exists($opTemp)) {
                $dlUrl = 'downloads/' . basename($opTemp);
            } else {
                $err[] = "Conversion completed but file not found.";
            }
        } else {
